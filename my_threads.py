...
class ThreadCubicFall(threading.Thread):
    ...
    def __init__(self, screen, game_settings, fall_cubics, dead_cubics, temp_cubics, thread_lock, black_lines, \
                 score_board):
        super(ThreadCubicFall, self).__init__()
        self.screen = screen
        self.game_settings = game_settings
        self.fall_cubics = fall_cubics
    ...
    def run(self):
        while not self.game_settings.exit_threads_flag:
            ...
            gf.key_down_update_cubics(self.screen, 'down', self.game_settings, self.fall_cubics, self.dead_cubics, \
    ...
    def run(self):
        while not self.game_settings.exit_threads_flag:
            ...
            while (self.game_settings.key_down) and (time.time() - self.game_settings.key_down_timestamp > 0.5):
                gf.key_down_update_cubics(self.screen, pygame.K_DOWN, self.game_settings, self.fall_cubics, \
                                          self.dead_cubics, self.temp_cubics, self.thread_lock, self.black_lines, \
                                          self.score_board)
                sleep(0.03)
            if self.game_settings.exit_threads_flag: return

            while (self.game_settings.key_left) and (time.time() - self.game_settings.key_left_timestamp > 0.5):
                gf.key_down_update_cubics(self.screen, pygame.K_LEFT, self.game_settings, self.fall_cubics, \
                                          self.dead_cubics, self.temp_cubics, self.thread_lock, self.black_lines, \
                                          self.score_board)
                sleep(0.08)
            if self.game_settings.exit_threads_flag: return
            ...
            while (self.game_settings.key_right) and (time.time() - self.game_settings.key_right_timestamp > 0.5):
                gf.key_down_update_cubics(self.screen, pygame.K_RIGHT, self.game_settings, self.fall_cubics, \
                                          self.dead_cubics, self.temp_cubics, self.thread_lock, self.black_lines, \
                                          self.score_board)
                sleep(0.08)
            if self.game_settings.exit_threads_flag: return