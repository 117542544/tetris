...
class ThreadCubicFall(threading.Thread):
    ...
    def __init__(self, game_settings, all_cubics, dead_cubics, temp_cubics, thread_lock):
    ...
    def run(self):
        while not self.game_settings.exit_threads_flag:
            ...
            gf.key_down_update_cubics('down', self.game_settings, self.all_cubics, self.dead_cubics, \
                                      self.temp_cubics, self.thread_lock)
    ...
    def run(self):
        while not self.game_settings.exit_threads_flag:
            ...
            while (self.game_settings.key_down) and (time.time() - self.game_settings.key_down_timestamp > 0.5):
                gf.key_down_update_cubics(pygame.K_DOWN, self.game_settings, self.all_cubics, self.dead_cubics, \
                                          self.temp_cubics, self.thread_lock)
                sleep(0.03)

            while (self.game_settings.key_left) and (time.time() - self.game_settings.key_left_timestamp > 0.5):
                gf.key_down_update_cubics(pygame.K_LEFT, self.game_settings, self.all_cubics, self.dead_cubics, \
                                          self.temp_cubics, self.thread_lock)
                sleep(0.08)

            while (self.game_settings.key_right) and (time.time() - self.game_settings.key_right_timestamp > 0.5):
                gf.key_down_update_cubics(pygame.K_RIGHT, self.game_settings, self.all_cubics, self.dead_cubics, \
                                          self.temp_cubics, self.thread_lock)
                sleep(0.08)